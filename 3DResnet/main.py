...
def get_opt():
    ...
    if opt.distributed:
        ...
        if opt.dist_rank == 0:
            print(opt)
            #check result path exists or not, if not, create it
            if not opt.result_path.exists():
                opt.result_path.mkdir(parents=True)
...
def get_train_utils(opt, model_parameters):
    ...
    if opt.optimizer == 'sgd':
        ...
        raise ValueError('Unknown optimizer: {}'.format(opt.optimizer))

    assert opt.lr_scheduler in ['plateau', 'multistep', 'warmup']
    assert not (opt.lr_scheduler == 'plateau' and opt.no_val)
    if opt.lr_scheduler == 'plateau':
        scheduler = lr_scheduler.ReduceLROnPlateau(
            optimizer, 'min', patience=opt.plateau_patience)
    elif opt.lr_scheduler == 'warmup':
        scheduler = get_linear_schedule_with_warmup(
            optimizer, num_warmup_steps=opt.warmup_epochs, num_training_steps=opt.n_epochs)
    else:
...
def main_worker(index, opt):
    ...
    if opt.loss_weights is not None:
        loss_weights = opt.loss_weights.split(',')
        loss_weights = [float(item) for item in loss_weights]
    ...
    for i in range(opt.begin_epoch, opt.n_epochs + 1):
        ...
        if not opt.no_train and opt.lr_scheduler in ('multistep', 'warmup'):
            scheduler.step()
