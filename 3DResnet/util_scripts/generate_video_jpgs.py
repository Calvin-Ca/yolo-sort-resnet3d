...
def video_process(video_file_path: Path, dst_root_path: Path, ext, fps=-1, size=240):
    ...
    label = video_file_path.parent.name
    dst_dir_path = dst_root_path / f"{label}_{name}"
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default='',
        type=str,
        help='Dataset name (kinetics | mit | ucf101 | hmdb51 | activitynet | self)')
    parser.add_argument(
    ...
    if args.dataset in ['kinetics', 'mit', 'activitynet', 'self']:
        ext = '.mp4'
    else:
        ext = '.avi'
